...
app.get('/api/health', async (_req, res) => {
  try {
    ...
    res.json({ ok: true, db: (rows as any)[0].result });
  } catch (e) {
...
app.get('/api/products', async (req, res) => {
  const { q, category, brand, ageMin, ageMax, minPrice, maxPrice, featured, ids } = req.query as Record<string, string>;
  ...
  if (ids) {
    ...
    try {
      ...
      return res.json(normalizeProducts(rows as any[]))
    } catch (e) {
  ...
  if (ageMin) { conditions.push('p.age_min >= ?'); params.push(Number(ageMin)); }
  if (ageMax) { conditions.push('p.age_max <= ?'); params.push(Number(ageMax)); }
  ...
  try {
    ...
    res.json(normalizeProducts(rows as any[]));
  } catch (e) {
...
app.get('/api/products/:slug', async (req, res) => {
  try {
    ...
    const [reviews] = await pool.query(
      `SELECT r.*, u.full_name FROM reviews r JOIN users u ON r.user_id = u.id WHERE r.product_id = ? ORDER BY r.created_at DESC`,
      [product.id]
    );
...
app.post('/api/cart/add', async (req, res) => {
  ...
  if (!sessionId || !productId || !quantity) return res.status(400).json({ error: 'Missing fields' });
...
app.post('/api/cart/remove', (req, res) => {
  ...
  if (!sessionId || !productId) return res.status(400).json({ error: 'Missing fields' });
...
app.post('/api/checkout', async (req, res) => {
  ...
  try {
    ...
    for (const item of cart) {
      ...
      if (!prod || prod.inventory < item.quantity) throw new Error('Out of stock');
      total += prod.price * item.quantity;
    }
    const [orderResult] = await conn.query(
      'INSERT INTO orders (user_id, status, total, shipping_address, payment_method) VALUES (?, "pending", ?, ?, ?)',
      [userId, total, JSON.stringify(shippingAddress || {}), paymentMethod || 'cod']
    );
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    res.json({ ok: true, userId: (result as any).insertId });
  } catch (e) {
    res.status(400).json({ ok: false, error: (e as Error).message });
  }
});
